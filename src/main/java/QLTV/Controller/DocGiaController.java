...
public class DocGiaController {
    ...
    private void fillTable(List<DocGia> list) {
        ...
        // Đảm bảo đã có dữ liệu khoa/lớp để map mã -> tên
        if (listKhoa == null) listKhoa = khoaDAO.findAll();
        if (listLop == null)  listLop  = lopDAO.findAll();

        for (DocGia dg : list) {
            String tenKhoa = tenKhoaByMa(dg.getMaKhoa());
            String tenLop  = tenLopByMa(dg.getMaLop());

            // Khớp đúng thứ tự cột của FormDocGia:
            // {MaDG, MaKhoa(ẩn), TenKhoa, MaLop(ẩn), TenLop, TenDG, NamSinh, GioiTinh, DiaChi, Email, Sdt}
            m.addRow(new Object[]{
                dg.getMaDG(),
                dg.getMaKhoa(),
                tenKhoa,
    ...
    private String tenKhoaByMa(String maKhoa) {
    ...
    private String tenLopByMa(String maLop) {
        if (maLop == null) return "";
        for (Lop l : listLop) {
            if (maLop.equals(l.getMaLop())) return l.getTenLop();
        }
        return "";
    }
    ...
    private void fillFormFromSelectedRow() {
        int viewRow = view.getTblDG().getSelectedRow();
        if (viewRow < 0) return;

        int row = view.getTblDG().convertRowIndexToModel(viewRow);
        DefaultTableModel m = view.getModel();

        String maDG   = safeStr(m.getValueAt(row, 0));
        String maKhoa = safeStr(m.getValueAt(row, 1));
        String maLop  = safeStr(m.getValueAt(row, 3));
        String ten    = safeStr(m.getValueAt(row, 5));
        String ns     = safeStr(m.getValueAt(row, 6));
        String gt     = safeStr(m.getValueAt(row, 7));
        String dc     = safeStr(m.getValueAt(row, 8));
        String email  = safeStr(m.getValueAt(row, 9));
        String sdt    = safeStr(m.getValueAt(row, 10));
